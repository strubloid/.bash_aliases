...
# This will be creating new tests in rafael
function rafael-create-test() {

  # Getting the current branch ID
  CurrentBranchName=$(git branch --show-current | grep -Po 'RAFA?-[0-9]*.*')

  # Getting the RAFA-[0-9]+
  rafaelBranchNumber=$(git branch --show-current | grep -Po 'RAFA?-[0-9]*')

  # Getting the current integer of the rafael branch
  rafaelCurrentNumber=$(git branch --show-current | grep -Po 'RAFA?-[0-9]*' | grep -Po '[0-9]*' | awk '{ print $1}')

  # This will return the lower case text after RAFA-15-populate-part-of-field => populate-part-of-field
  BranchNameIgnoringJiraTicket=$(git branch --show-current | grep -Po '(RAFA?-[0-9]*)(.*)' | grep -Po '(-[a-zA-Z].*)' | grep -Po '([a-zA-Z].*)')
  ...
  echo "======================================================================================="
  echo "rafael Test Builder "
  echo "V1.0"
  echo "Creating: Rule"
  echo "======================================================================================="
  echo "[Current Branch Name]: $CurrentBranchName"
  echo "[rafael Branch Number]: $rafaelBranchNumber"
  echo "[Current Number]: $rafaelCurrentNumber"
  ...
  ## Creating test suite
  printf "[Test Suite]"
  TestSuiteFullPath="${TestSuiteFolder}/${rafaelBranchNumber}-suite.test.ts"
  printf "\n = $TestSuiteFullPath"
  printf "\n = touch: \t"
  ...
  for ((line = 1; line <= "$count"; line++)); do
    ...
    if [ "$line" -eq "1" ]; then
      echo "import SuiteConfiguration from \"src/rafael/SuiteConfiguration.interface\";" >> "$TestSuiteFullPath"
      echo "import rafaelTestSuite from \"src/rafael/rafaelTestSuite\";" >> "$TestSuiteFullPath"
    fi
    ...
    if [ "$line" -eq "$count" ]; then
      ...
      printf "\ndescribe('$rafaelBranchNumber', () => {" >> "$TestSuiteFullPath"
      ...
      printf "\n\t\t}];" >> "$TestSuiteFullPath"
      printf "\n\t\tnew rafaelTestSuite(allTests).runAll();" >> "$TestSuiteFullPath"
...
DefaultTest=$(cat << END
import rafaelTests from "src/rafael/rafaelTests";
import rafaelDefaults from "../rafael-tests/src/rafael/rafaelDefaults";
import rafaelActions from "src/rafael/rafaelActions";

/**
 * This is the main class that does all the tests for our rafaelTests cases
 */
export default class RenameClassName extends rafaelTests {
    ...
    public static validate = async (page, browser) => {
        ...
        await rafaelDefaults.goTo(page, rafaelActions.getPageUrl(

...
def main():
    ...
    # Now process the trimmed VTT file
    try:
        transcription = VTTProcessor.get_trimmed_transcription(vtt_file, from_time)
        if debug:
            print("\nFirst 500 characters of processed transcription:")
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        summary = openai_service.ask([{"role": "user", "content": prompt}],  model="gpt-3.5-turbo-0125")
    ...
    ai_response = openai_service.ask([{"role": "user", "content": final_prompt}], model="gpt-5-nano")
